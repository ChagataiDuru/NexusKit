document.addEventListener('DOMContentLoaded', () => {
    ...
    mediaUpload.addEventListener('change', async (event) => {
        ...
        try {
            const response = await fetch('/api/v1/ffmpeg/upload', {
                method: 'POST',
                body: formData
            });
    ...
    convertBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('/api/v1/ffmpeg/convert', {
                method: 'POST',
                headers: {
            ...
            const data = await response.json();
            pollConversionStatus(currentTaskId);
    ...
    function pollConversionStatus(taskId) {
        const interval = setInterval(async () => {
            try {
                const response = await fetch(`/api/v1/ffmpeg/status/${taskId}`);
                const data = await response.json();

                if (data.status === 'completed') {
                    clearInterval(interval);
                    statusMessages.innerHTML = `<div class="alert alert-success">Conversion complete! <a href="${data.download_url}" download>Click here to download</a></div>`;
                } else if (data.status === 'failed') {
                    clearInterval(interval);
                    statusMessages.innerHTML = `<div class="alert alert-danger">Conversion failed: ${data.error}</div>`;
                } else {
                    statusMessages.innerHTML = `<div class="alert alert-info">Conversion in progress... Progress: ${data.progress || 0}%</div>`;
                }
            } catch (error) {
                clearInterval(interval);
                statusMessages.innerHTML = `<div class="alert alert-danger">Error checking conversion status.</div>`;
            }
        }, 3000);
    }
});