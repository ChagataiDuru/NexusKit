document.addEventListener('DOMContentLoaded', () => {
    ...
    fetchInfoBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('/api/v1/ytdl/fetch-info', {
                method: 'POST',
                headers: {
    ...
    downloadBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('/api/v1/ytdl/download-request', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    function pollTaskStatus(taskId) {
        const interval = setInterval(async () => {
            try {
                const response = await fetch(`/api/v1/ytdl/task-status/${taskId}`);
                const data = await response.json();
                ...
                if (data.status === 'completed') {
                    ...
                    clearInterval(interval);
                    statusMessages.innerHTML = `<div class="alert alert-danger">Download failed: ${data.result}</div>`;
                } else {
                    statusMessages.innerHTML = `<div class="alert alert-info">Download in progress...</div>`;
                }
            } catch (error) {
                clearInterval(interval);
                statusMessages.innerHTML = `<div class="alert alert-danger">Error checking task status.</div>`;
            }
        }, 3000);
    }
});
